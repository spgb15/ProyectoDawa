...
export const login = async (req, res) => {
    try {
        ...
        const [respuesta] = await db_pool_connection.query("SELECT cedula , rol FROM usuarios WHERE cedula =  ? AND password = ?", [cedula, password]);

        if (respuesta.length > 0) {
            const user = respuesta[0];
            return res.status(200).json(response_sucess({role: user.rol}, 'Login Exitoso'));
        } else {
